...
public:
	...
	CronExpression(const std::string& input, DATA_TYPE data_type)
	{
		...
		{
			std::pair<int, int> pair_range = get_range_from_type(data_type);

			std::vector<std::string> v;
			UtilLib::Text::split(v, input, CRON_SEPERATOR_ENUM);
		...
		else if (input.find_first_of(CRON_SEPERATOR_RANGE) != std::string::npos)
		{
			...
			std::vector<std::string> v;
			UtilLib::Text::split(v, input, CRON_SEPERATOR_RANGE);
			...
			int from = atoi(v[0].data());
			int to = atoi(v[1].data());
		...
		else if (input.find_first_of(CRON_SEPERATOR_INTERVAL) != std::string::npos)
		{
			...
			std::vector<std::string> v;
			UtilLib::Text::split(v, input, CRON_SEPERATOR_INTERVAL);
...
private:
	...
	static std::pair<int, int> get_range_from_type(DATA_TYPE data_type)
	{
		int from = 0;
		int to = 0;
...
#define CRON_TIME_IS_VALID(p) ((p) != nullptr && (p)->is_valid())
#define CRON_TIME_GET_ERROR(p) ((p) == nullptr ? "nullptr" : (p)->get_error())
...
public:
	CronTime(const std::string& time_string)
	{
		...
		std::vector<std::string> v;
		UtilLib::Text::split(v, time_string, ' ');
		if (v.size() != 7)
			return;
...
class CronTimerMgr : public Singleton<CronTimerMgr>
{
public:
	CronTimerMgr()
	{
	...
	void start()
	{
		...
		thread_ = new std::thread([this]() {this->thread_proc(); });
	}
	...
	int add_timer(const std::string& timer_string, const CRON_FUNC_CALLBACK& func)
	{
		++latest_timer_id_;
		CronTimer* timer_ptr = new CronTimer(latest_timer_id_, timer_string, func);

		if (!timer_ptr->cron_time.is_valid())
		{
			delete timer_ptr;
			return 0;
		}
	...
	struct CronTimer
	{
		CronTimer(int timer_id_r, const std::string& timer_string_r, const CRON_FUNC_CALLBACK& func_r)
			: timer_id(timer_id_r)
...
private:
	...
	std::list<CronTimer*> cron_timers_;
